...
async function main() {
    ...
    const [electronicsId, mensFashionId, womensFashionId, homeLivingId, sportsId, beautyId] = createdCategories.map(c => c.id);
    // 2. Create Products
    const products = [
        // Electronics
        {
    ...
    for (const p of products) {
        ...
        const product = await prisma.product.create({
            data: {
                ...
                sku: 'SKU-' + (0, uuid_1.v4)().substring(0, 8).toUpperCase(),
                images: {
                    create: {
                        url: p.imageUrl,
                        altText: p.title
